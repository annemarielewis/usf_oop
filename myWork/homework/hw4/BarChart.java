...
//            chart.draw();
//            StdDraw.show();
//            StdAudio.play("foo.wav");
//        }
//    }
//
// hardcode replaced with grabbing from "cities" file --> This was pretty confusing to me to figure out,
// so I relied heavily on chatgpt help after my solo attempts had words writing over each other in the graph (because of using this.chart in instantiating, it'd seem!)
// NOTE, written after completion: In step 2, I thought it was asking me to do everything in steps 3-5. so that functionality is here as well!
        public static void main(String[] args) throws FileNotFoundException {
    final String title  = "The Most Populous Cities in the World";
    final String xAxis  = "Population (thousands)";
    final String source = "Sources: SEDAC; United Nations; Demographia";

    File file = new File("data/cities.txt");
    Scanner scanner = new Scanner(file);

    // skip header lines in the file
    for (int i = 0; i < 3 && scanner.hasNextLine(); i++) scanner.nextLine();
    ...
    while (scanner.hasNextLine()) {
        ...
        int count = Integer.parseInt(countLine); //12

        // brand-new chart each frame (no stale data-->no text over text)
        BarChart chart = new BarChart(title, xAxis, source);
        ...
        for (int i = 0; i < count && scanner.hasNextLine(); i++) {
            ...
            chart.add(name, value, category); //initiating object
        }

        chart.setCaption(year);

        // clear the offscreen buffer before drawing
